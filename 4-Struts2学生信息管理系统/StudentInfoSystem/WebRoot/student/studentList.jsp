...
<script type="text/javascript">
	...
	function loadClassInfo(){
		...
		//ajax
		$.post("class!getClassByGradeId",{s_gradeId:s_gradeId},//获取对应年级下所有班级
			function(result){
				$("#s_classId").val("-1");
				var result=eval("("+result+")");
		...
		<table class="table table-bordered table-hover">
			...
			<c:forEach var="student" items="${studentList }" varStatus="status">
				<tr>
					...
					<td>
						<button class="btn btn-mini btn-info" type="button" onclick="javascript:window.location='student!show?studentId=${student.studentId}'">查看详细信息</button>&nbsp;&nbsp;
						<button class="btn btn-mini btn-info" type="button" onclick="javascript:window.location='student!preSave?studentId=${student.studentId}'">修改</button>&nbsp;&nbsp;
						<button class="btn btn-mini btn-danger" type="button" onclick="studentDelete('${student.studentId}')">删除</button>
					</td>
				</tr>
			</c:forEach>
		</table>
